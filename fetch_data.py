...
for _, row in raw_df.iterrows():
    ...
    vec = {
        ...
        "is_human": 1.0 if "human" in species and "alien" not in species else 0.0,
        "is_mutant": 1.0 if "mutant" in species else 0.0,
        "is_alien_or_god": 1.0 if any(
            k in species for k in ["alien", "god", "kryptonian", "martian", "asgardian", "new god"]
        ) else 0.0,
        ...
        "skin_non_human": 1.0 if any(
            k in skin for k in ["green", "blue", "metal", "purple", "stone", "chalk", "gold"]
        ) else 0.0,
        ...
        "power_tech_gadgets": 1.0 if any(k in power_type for k in ["tech", "gadget"]) or any(k in powers_desc for k in ["gadget", "suit", "device", "web-shooter"]) else 0.0,
...
print(f"Done! Cleanly generated '{OUTPUT_FILE}' with {len(df_clean)} characters.")